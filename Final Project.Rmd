...
Based on the analysis above, I built this multilevel model:

```{r, warning=FALSE, message=FALSE}
model_1 <- lmer(salary ~ yearsofexperience +stockgrantvalue + cityid + education + gender
              + (1 + yearsofexperience + stockgrantvalue + cityid + education | title) 
              + (1 | race), 
              data = cor, REML = F)
```
...
```{r echo=FALSE, message=FALSE, fig.cap="Correlation Graph", fig.width=8, fig.height=4}
qq <- qqmath(model_1)
ml_ranef <- ranef(model_1)

plot_grid(qq, labels = NULL)
plot_grid(ml_ranef, labels = NULL)
```
